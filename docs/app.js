...
function provenance() {
  const byCo = new Map(); for (const c of META.calls) { if (!byCo.has(c.company)) byCo.set(c.company, []); byCo.get(c.company).push(c); }
  const groups = [...byCo].map(([co, cs], i) => [i ? ' · ' : '', `${co} `, cs.map((c, j) => [j ? ', ' : '', h('a', { href: c.source_page, rel: 'noopener', title: `call ${c.call_date}` }, c.label)])]);
  return h('p', { class: 'provenance' }, h('b', {}, `Real data, ${META.calls.length} calls: `), groups, `. Scored from each company's own transcript by ${META.model}, run ${META.run_date}. EPS estimates and prices: Yahoo Finance.`);
}
...
function renderPage() {
  ...
  $('#pill-long').textContent = S.custom ? ' · loaded in this tab' : ` · ${new Set(META.calls.map(c => c.ticker)).size} companies · ${META.calls.length} calls · ${META.model} · run ${META.run_date}`;
  ...
  main.append(h('div', { class: 'ticker-tabs', role: 'tablist', 'aria-label': 'Companies' }, tickers.map(t => h('button', { role: 'tab', id: 'tab-' + t, 'aria-controls': 'ticker-panel', 'aria-selected': t === S.ticker ? 'true' : 'false', onclick: () => { if (t === S.ticker) return; S.ticker = t; history.pushState(null, '', '#' + encodeURIComponent(t)); render(); } }, t)), h('span', { class: 'meta' }, `${qs.length} ${qs.length === 1 ? 'quarter' : 'quarters'} · ${tickers.length} tickers · ${calls} calls · `, recSummary(data, tickers, calls))));
  ...
  const delta = v => qs.length < 2 ? null : h('small', {}, `${v > 0 ? '▲' : v < 0 ? '▼' : '·'} ${tone(v)} vs prior`);   // one quarter: nothing to compare with
  ...
  panel.append(h('div', { class: 'panels' },
    h('div', { class: 'chart' }, h('div', { class: 'ch-head' }, h('h2', {}, 'Multi-quarter sentiment trajectory'), h('p', {}, `Management prepared remarks vs analyst Q&A · ${qs.length} ${qs.length === 1 ? 'quarter' : 'quarters'} · shaded band = framing gap`)), trajectory(qs), h('div', { class: 'legend' }, h('span', {}, h('i', { style: 'background:var(--viz-1)' }), 'Management'), h('span', {}, h('i', { style: 'background:var(--viz-2)' }), 'Analyst Q&A'), h('span', {}, h('i', { style: 'background:var(--viz-1);opacity:.25;height:8px' }), 'Framing gap'))),
    h('div', { class: 'chart' }, h('div', { class: 'ch-head' }, h('h2', {}, 'Topic emphasis'), h('p', {}, `${cur.label} · themes extracted by the model; bar = share of airtime`)), h('div', { class: 'topics' }, c.topics.length ? c.topics.map(t => { const g = t.mgmt - t.qa, one = t.mgmt_missing === true || t.qa_missing === true; return h('div', { class: 'topic' }, h('div', { class: 'head' }, h('b', {}, t.name), h('span', { class: 'mono muted' }, `${(t.weight * 100).toFixed(0)}% of call`)), h('div', { class: 'bar' }, h('i', { style: `width:${Math.min(100, t.weight / maxW * 100)}%` })), h('div', { class: 'tones' }, t.mgmt_missing === true ? h('span', { class: 'muted', title: 'not a topic of the prepared remarks' }, 'M —') : h('span', { class: cls(t.mgmt) }, 'M ' + tone(t.mgmt)), t.qa_missing === true ? h('span', { class: 'muted', title: 'not a topic of the Q&A' }, 'Q —') : h('span', { class: cls(t.qa) }, 'Q ' + tone(t.qa)), !one && Math.abs(g) > 0.25 ? h('span', { class: 'tag tag-warning' }, 'gap ' + tone(g)) : null)); }) : h('p', { class: 'muted' }, 'No topics in this file.')), h('p', { class: 'hint', style: 'margin-top:12px' }, 'Wide gaps show where analysts push back against management framing.'))));
  const r = pearson(qs.map(q => q.mgmt - q.qa), qs.map(q => q.residual_5d));
  panel.append(h('div', { class: 'panels-2' },
    h('div', { class: 'chart' }, h('div', { class: 'ch-head' }, h('h2', {}, 'Sentiment gap vs residual return'), h('p', {}, `5-day post-call return after controlling for sector and EPS surprise · one point per call · Pearson r = ${r === null ? 'n/a' : r.toFixed(2)} over n = ${qs.length} calls${qs.length < 30 ? ` — far too few points for r to show a relationship; it is here to show the method, not a finding` : ' (not a trading signal)'}`)), scatter(qs), h('p', { class: 'hint', style: 'margin-top:8px' }, 'Quadrant of interest: wide gap and negative residual — management rosy, market unconvinced.')),
