...
function render() {
  ...
  main.append(h('div', { class: 'panels' },
    h('div', { class: 'chart' }, h('div', { class: 'ch-head' }, h('h3', {}, 'Multi-quarter sentiment trajectory'), h('p', {}, `Management prepared remarks vs analyst Q&A · ${qs.length} quarters · shaded band = framing gap`)), trajectory(qs), h('div', { class: 'legend' }, h('span', {}, h('i', { style: 'background:var(--viz-1)' }), 'Management'), h('span', {}, h('i', { style: 'background:var(--viz-2)' }), 'Analyst Q&A'), h('span', {}, h('i', { style: 'background:var(--viz-1);opacity:.25;height:8px' }), 'Framing gap'))),
    h('div', { class: 'chart' }, h('div', { class: 'ch-head' }, h('h3', {}, 'Topic emphasis'), h('p', {}, `${cur.label} · themes extracted by the model; bar = share of airtime`)), h('div', { class: 'topics' }, c.topics.length ? c.topics.map(t => { const g = t.mgmt - t.qa; return h('div', { class: 'topic' }, h('div', { class: 'head' }, h('b', {}, t.name), h('span', { class: 'mono muted' }, `${(t.weight * 100).toFixed(0)}% of call`)), h('div', { class: 'bar' }, h('i', { style: `width:${Math.min(100, t.weight / maxW * 100)}%` })), h('div', { class: 'tones' }, h('span', { class: cls(t.mgmt) }, 'M ' + tone(t.mgmt)), h('span', { class: cls(t.qa) }, 'Q ' + tone(t.qa)), Math.abs(g) > 0.25 ? h('span', { class: 'tag tag-warning' }, 'gap ' + tone(g)) : null)); }) : h('p', { class: 'muted' }, 'No topics in this file.')), h('p', { class: 'hint', style: 'margin-top:12px' }, 'Wide gaps show where analysts push back against management framing.'))));
  const r = pearson(qs.map(q => q.mgmt - q.qa), qs.map(q => q.residual_5d));
  main.append(h('div', { class: 'panels-2' },
    h('div', { class: 'chart' }, h('div', { class: 'ch-head' }, h('h3', {}, 'Sentiment gap vs residual return'), h('p', {}, `5-day post-call return after controlling for sector and EPS surprise · one point per call · Pearson r = ${r === null ? 'n/a' : r.toFixed(2)} (n = ${qs.length}, illustrative, not a signal)`)), scatter(qs), h('p', { class: 'hint', style: 'margin-top:8px' }, 'Quadrant of interest: wide gap and negative residual — management rosy, market unconvinced.')),
